...
if page == "Home Page":
    # # Create buttons to navigate to other pages arranged horizontally
    # col1, col2 = st.columns(2)

    # if col1.button("Schemes"):
    #     page = "Select Scheme"

    # if col2.button("Visualization"):
    #     page = "Visualization"
    ...
    if selected_fund == 'Bond Fund':
        ...
        year_bond = st.number_input('Year', min_value=0.0)
        month_bond = st.number_input('Month', min_value=0.0)
        day_bond = st.number_input('Day', min_value=0.0)
        weekday_bond = st.number_input('Weekday', min_value=0.0)


        # Model loading and prediction for Bond Fund
        st.subheader('Prediction Result for Bond Fund:')
        
        # Shift the 'Sqrt_NAV' values forward by the specified number of months to create the target variable
        bond_data['Target_NAV'] = bond_data['Sqrt_NAV'].shift(-21 * forecast_months)  # Assuming 21 trading days per month
        bond_data['Target_NAV'].fillna(method='ffill', inplace=True)
    
        X_bond = bond_data[['Log_ONU', 'Nav Per Unit', 'Sale Price per Unit', 'Repurchase Price/Unit', 'Year', 'Month', 'Day', 'Weekday']]
        y_bond = bond_data['Target_NAV']
        ...
        predicted_sqrt_nav_bond = ensemble_model_bond.predict([[log_onu_bond, nav_per_unit_bond, sale_price_per_unit_bond, repurchase_price_per_unit_bond, year_bond, month_bond, day_bond, weekday_bond]])[0]
        ...
        year_jikimu = st.number_input('Year', min_value=0.0)
        month_jikimu = st.number_input('Month', min_value=0.0)
        day_jikimu = st.number_input('Day', min_value=0.0)
        weekday_jikimu = st.number_input('Weekday', min_value=0.0)

        # Model loading and prediction for Jikimu Fund
        st.subheader('Prediction Result for Jikimu Fund:')
        
        # Shift the 'Sqrt_NAV' values forward by the specified number of months to create the target variable
        jikimu_data['Target_NAV'] = jikimu_data['Sqrt_NAV'].shift(-21 * forecast_months)  # Assuming 21 trading days per month
        jikimu_data['Target_NAV'].fillna(method='ffill', inplace=True)
        
        X_jikimu = jikimu_data[['Log_ONU', 'Nav Per Unit', 'Sale Price per Unit', 'Repurchase Price/Unit', 'Year', 'Month', 'Day', 'Weekday']]
        y_jikimu = jikimu_data['Target_NAV']
        ...
        predicted_sqrt_nav_jikimu = ensemble_model_jikimu.predict([[log_onu_jikimu, nav_per_unit_jikimu, sale_price_per_unit_jikimu, repurchase_price_per_unit_jikimu, year_jikimu, month_jikimu, day_jikimu, weekday_jikimu]])[0]
        ...
        year_watoto = st.number_input('Year', min_value=0.0)
        month_watoto = st.number_input('Month', min_value=0.0)
        day_watoto = st.number_input('Day', min_value=0.0)
        weekday_watoto = st.number_input('Weekday', min_value=0.0)

        # Model loading and prediction for Watoto Fund
        st.subheader('Prediction Result for Watoto Fund:')
        
        # Shift the 'Sqrt_NAV' values forward by the specified number of months to create the target variable
        watoto_data['Target_NAV'] = watoto_data['Sqrt_NAV'].shift(-21 * forecast_months)  # Assuming 21 trading days per month
        watoto_data['Target_NAV'].fillna(method='ffill', inplace=True)
        
        X_watoto = watoto_data[['Log_ONU', 'Nav Per Unit', 'Sale Price per Unit', 'Repurchase Price/Unit', 'Year', 'Month', 'Day', 'Weekday']]
        y_watoto = watoto_data['Target_NAV']
        ...
        predicted_sqrt_nav_watoto = ensemble_model_watoto.predict([[log_onu_watoto, nav_per_unit_watoto, sale_price_per_unit_watoto, repurchase_price_per_unit_watoto, year_watoto, month_watoto, day_watoto, weekday_watoto]])[0]
        ...
        year_liquid = st.number_input('Year', min_value=0.0)
        month_liquid = st.number_input('Month', min_value=0.0)
        day_liquid = st.number_input('Day', min_value=0.0)
        weekday_liquid = st.number_input('Weekday', min_value=0.0)

        # Model loading and prediction for Liquid Fund
        st.subheader('Prediction Result for Liquid Fund:')
        
        # Shift the 'Sqrt_NAV' values forward by the specified number of months to create the target variable
        liquid_data['Target_NAV'] = liquid_data['Sqrt_NAV'].shift(-21 * forecast_months)  # Assuming 21 trading days per month
        liquid_data['Target_NAV'].fillna(method='ffill', inplace=True)
        
        X_liquid = liquid_data[['Log_ONU', 'Nav Per Unit', 'Sale Price per Unit', 'Repurchase Price/Unit', 'Year', 'Month', 'Day', 'Weekday']]
        y_liquid = liquid_data['Target_NAV']
        ...
        predicted_sqrt_nav_liquid = ensemble_model_liquid.predict([[log_onu_liquid, nav_per_unit_liquid, sale_price_per_unit_liquid, repurchase_price_per_unit_liquid, year_liquid, month_liquid, day_liquid, weekday_liquid]])[0]
        ...
        # Add input fields for Umoja Fund features
        ...
        year_umoja = st.number_input('Year', min_value=0.0)
        month_umoja = st.number_input('Month', min_value=0.0)
        day_umoja = st.number_input('Day', min_value=0.0)
        weekday_umoja = st.number_input('Weekday', min_value=0.0)

        # Model loading and prediction for Umoja Fund
        st.subheader('Prediction Result for Umoja Fund:')
        
        # Shift the 'Sqrt_NAV' values forward by the specified number of months to create the target variable
        umoja_data['Target_NAV'] = umoja_data['Sqrt_NAV'].shift(-21 * forecast_months)  # Assuming 21 trading days per month
        umoja_data['Target_NAV'].fillna(method='ffill', inplace=True)
        
        X_umoja = umoja_data[['Log_ONU', 'Nav Per Unit', 'Sale Price per Unit', 'Repurchase Price/Unit', 'Year', 'Month', 'Day', 'Weekday']]
        y_umoja = umoja_data['Target_NAV']
        ...
        predicted_sqrt_nav_umoja = ensemble_model_umoja.predict([[log_onu_umoja, nav_per_unit_umoja, sale_price_per_unit_umoja, repurchase_price_per_unit_umoja, year_umoja, day_umoja, month_umoja, weekday_umoja]])[0]
        ...
        nav_per_unit_wekeza_maisha = st.number_input('Nav Per Unit', min_value=0.0)
        sale_price_per_unit_wekeza_maisha = st.number_input('Sale Price per Unit', min_value=0.0)
        repurchase_price_per_unit_wekeza_maisha = st.number_input('Repurchase Price per Unit', min_value=0.0)
        year_wekeza_maisha = st.number_input('Year', min_value=0.0)
        month_wekeza_maisha = st.number_input('Month', min_value=0.0)
        day_wekeza_maisha = st.number_input('Day', min_value=0.0)
        weekday_wekeza_maisha = st.number_input('Weekday', min_value=0.0)

        # Model loading and prediction for Wekeza Maisha Fund
        st.subheader('Prediction Result for Wekeza Maisha Fund:')
        
        # Shift the 'Sqrt_NAV' values forward by the specified number of months to create the target variable
        wekeza_maisha_data['Target_NAV'] = wekeza_maisha_data['Sqrt_NAV'].shift(-21 * forecast_months)  # Assuming 21 trading days per month
        wekeza_maisha_data['Target_NAV'].fillna(method='ffill', inplace=True)
        
        X_wekeza_maisha = wekeza_maisha_data[['Log_ONU', 'Nav Per Unit', 'Sale Price per Unit', 'Repurchase Price/Unit', 'Year', 'Month', 'Day', 'Weekday']]
        ...
        predicted_sqrt_nav_wekeza_maisha = ensemble_model_wekeza_maisha.predict([[log_onu_wekeza_maisha, nav_per_unit_wekeza_maisha, sale_price_per_unit_wekeza_maisha, repurchase_price_per_unit_wekeza_maisha, year_wekeza_maisha, month_wekeza_maisha, day_wekeza_maisha, weekday_wekeza_maisha]])[0]
